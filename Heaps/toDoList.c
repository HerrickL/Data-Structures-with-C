...
** Author: L. Herrick
** Date: 5/23/15
** Description: Implenation file and for toDoList.  Includes helper 
functions: createTask, saveList, loadList, printList, deleteList, 
compare, and print_type.  Most implementations are drawn from ideas
...
Task* createTask (int priority, char *desc)
{
	...
	printf("Description is set to: %s\n", newTask->description);
...
{
    /* FIXME: Write this - done*/
	assert(sizeDynArr(heap)!=0);
	int i;
	DynArr *copyHeap;

	//create new heap
	copyHeap = createDynArr(sizeDynArr(heap));
	...
	//print
	i = sizeDynArr(copyHeap) - 1;
	while(i >= 0){
		print_type(getDynArr(copyHeap, i));
		i--;
	}

	//free new heap->data and new heap pointer
	freeDynArr(copyHeap);
	free(copyHeap);
}
...
int compare(TYPE left, TYPE right)
{
	...
	struct Task* leftTask;
	struct Task* rightTask;

	/*reassign void pointer to data pointer*/
	leftTask = (struct Task*)left;
	rightTask = (struct Task*)right;
