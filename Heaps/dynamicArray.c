...
** Author: L. Herrick
** Date: 5/23/15
** Description: Implenation file and for min heap using dynamic arrays.
File includes dynamic array implemention as well as heap functions:
_smallerIndexHeap, _adjustHeap, getMinHeap, addHeap, _adjustHeap, removeMinHeap, 
_buildHeap, sortHeap.  Most implementations are drawn from ideas
learned from CS 261 class worksheets 33, 34, and heaps practice.
*********************************************************************/
...
void addHeap(DynArr *heap, TYPE val)
{
	...
	/*iterate through array, making swaps when needed*/
	while(pos != 0){
		parent = (pos-1)/2;
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
   /* FIXME - done*/
	...
	if(rightChild < max){
		int smallest = _smallerIndexHeap(heap, leftChild, rightChild);
		...
		if(compare(getDynArr(heap,leftChild), getDynArr(heap, pos)) == -1){
...
void sortHeap(DynArr *heap)
{
   /* FIXME - done*/
	...
	for(i=sizeDynArr(heap)-1; i>0; i--){
		/*swap last element being considered with first*/
		swapDynArr(heap, 0, i);
		/*adjust heap up to but not including last element being considered*/
